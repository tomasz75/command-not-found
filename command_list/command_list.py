...
class command_list(object):
    ...
    def write_list(self, new_commit):
        ...
        new_command_list_file = "-".join(
            self.command_list_file.split("-")[:-1] + [new_commit + ".h"]
        )
    ...
    def get_list_from_deb(self, deb_path):
        ...
        all_content = [
            full_string.split()[5]
            for full_string in subprocess.run(
                ["dpkg", "-c", deb_path], capture_output=True
            )
            .stdout.decode("utf-8")
            .split("\n")[:-1]
        ]
        binaries = [
            os.path.basename(b.replace("./data/data/pl.sviete.dom/files/usr/bin/", ""))
            for b in all_content
            if b.startswith("./data/data/pl.sviete.dom/files/usr/bin/")
            and os.path.basename(
                b.replace("./data/data/pl.sviete.dom/files/usr/bin/", "")
            )
        ]
        return binaries
        return binaries
